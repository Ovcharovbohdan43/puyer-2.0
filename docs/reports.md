...
- **Period CSV (every plan):** From/To on `/reports` (default: first day of the current UTC month through today). On a phone the two date fields share one shrinking row. Download includes period totals (paid / outstanding / overdue among invoices *issued* in that window, dominant currency) plus one row per invoice. Filename `puyer-report-{from}-{to}.csv` with a UTF-8 BOM.
...
```
[2026-08-30] – Fixed: Period From/To shrink on a phone instead of overflowing the toolbar.
